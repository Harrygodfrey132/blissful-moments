...
const PersonalQuote: React.FC = () => {
  ...
  return (
    <div>
        ...
        <div className="flex items-center md:gap-8 gap-3">
          <div className="relative inline-block w-12 align-middle select-none transition-all duration-200 ease-in">
            ...
            <label
              ...
              className={`toggle-label block overflow-hidden  md:h-8 h-6 md:!w-14 !w-12 bg-blue-light-900 rounded-full cursor-pointer transition-all duration-200 ease-in-out ${isEnabled ? "bg-blue-light-900" : "bg-gray-300"}`}
            ></label>
          </div>
      ...
      {(isEnabled) && (
        <div>
          <h2 className="md:text-4xl text-3xl mx-auto !max-w-4xl text-center font-medium font-playfair mb-5 relative">
            <RiDoubleQuotesL className="text-blue-light-900 absolute top-2 left-1" />
            <div
              className="border w-full border-dashed bg-[#f8f8f8] text-blue-900 px-6 py-6 border-gray-300 focus:outline-none focus:border-gray-500"
              contentEditable
          ...
          <div className="w-full mb-4 m-auto text-center">
            <button onClick={generateQuote} className="md:text-2xl text-lg font-playfair border border-gray-300 font-medium px-4 py-2 bg-[#F3EAEACC] text-blue-light-900 suggest-btn rounded-lg">
