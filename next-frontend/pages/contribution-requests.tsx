                                                    ...
                                                    <table className="min-w-full divide-y divide-gray-300">
                                                        <thead className="bg-gray-50">
                                                            <tr>
                                                                ...
                                                                <th className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Posted Details</th>
                                                            ...
                                                            {requests.length === 0 ? (
                                                                ...
                                                                requests.map((contributionRequest, index) => (
                                                                    <tr key={contributionRequest.id}>
                                                                        ...
                                                                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{contributionRequest.full_name}</td>
                                                                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{contributionRequest.email}</td>
                                                                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{formatDate(contributionRequest.created_at)}</td>
                                                                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                                            <FaEye className="text-blue-500 cursor-pointer" onClick={() => handleViewDetails(contributionRequest)} />
                                                                        </td>
                                                                        <td className="whitespace-nowrap px-3 py-4 text-sm">
                                                                            <span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${contributionRequest.status === 0 ? 'bg-yellow-100 text-yellow-800' : contributionRequest.status === 1 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                                                                {contributionRequest.status === 0 ? 'Pending' : contributionRequest.status === 1 ? 'Accepted' : 'Declined'}
                                                                            </span>
                                                                        </td>
                                                                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
            ...
            {isModalOpen && currentContribution && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white rounded-lg shadow-lg p-6 max-w-lg w-full">
                        ...
                        <p><strong>Name:</strong> {currentContribution.name}</p>
