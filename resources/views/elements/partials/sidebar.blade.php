<aside class="flex flex-col w-[300px]" :class="{ 'hidden sm:flex sm:flex-col': window.outerWidth > 768 }">
    ...
    <div class="flex-grow flex flex-col justify-between text-white bg-gray-950">
        <nav class="flex flex-col mx-4 my-6 space-y-4">
            <a href="{{ route('dashboard') }}"
                class="inline-flex items-center py-3 rounded px-2 {{ request()->is('dashboard') ? 'text-black bg-white' : '' }}"
            ...
            <a href="{{ route('users.listing') }}"
                class="inline-flex items-center py-3 hover:text-white hover:bg-gray-700 focus:text-gray-400 focus:bg-gray-700 rounded-lg px-2 {{ request()->is('users*') ? 'text-black bg-white' : 'text-white' }}"
                :class="{ 'justify-start': menu, 'justify-center': menu == false }">
                <svg aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="h-6 w-6">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
